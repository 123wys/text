  ...
  <div>
    <div class="fullTable">
      <div class="searchTitle">
        ...
        <span class="s-text">关键词:</span><input v-model="searchTxt" class="s-input" type="text" placeholder="站点/页面名称/修改人" style="width: 170px;">
      ...
      <div class="dialogWrapper" v-show="isShowDialog">
        ...
        <div class="dialog" v-loading="!isFinished">
          ...
          <el-form :model="form">
            ...
            <el-form-item label="页面ID" :label-width="formLabelWidth" v-show="!isAdd">
...
export default {
  ...
  methods: {
    ...
    async getStateList() {
      try {
        let data = await pageConfigSites({
    ...
    async getQueryPages() {
      try {
        this.loading = true;
        let data = await pageConfigQuery({
          appSource: this.appSource,
    ...
    async doUpdatePage() {
      try {
        this.isFinished = false;
        let data = await pageConfigUpdate({
          stateId: this.form.stateId,
    ...
    async doDeletePages(id) {
      try {
        let data = await pageConfigDelete({
          pageId: id
        });
        if (data.success) {
          this.getQueryPages();
    ...
    goDownload() {
      let url = `/track/page_config/query_export.json?appSource=${
        this.appSource
      }&pageId=${this.pageId}&modifier=${this.modifier}&search=${
        this.searchTxt
      }`;
      Download(url);
    },
