  ...
  <div>
    <div class="searchTitle clearfix">
      ...
      <el-select class="s-select urlSelect" v-model="url" filterable placeholder="请选择">
        <el-option v-for="item in options" :label="item.urlName +'(' + item.url +')'" :value="item.url" :key="item.url">
        </el-option>
      </el-select>
...
import { rootUrl, rootData, nextData } from "../../../..//service/getData";
import download from "../../svg/download";
import { Download, formatDate } from "../../../../config/mUtils";

export default {
  name: "hingeDestinationZaWeb",
  data() {
    return {
      options: [],
      url: "",
  ...
  methods: {
    ...
    async getRootUrl() {
      try {
        let data = await rootUrl({ appSource: this.appSource });
        if (data.success) {
    ...
    goDownload() {
      let url = `/analysis/web_path_flow/export.json?appSource=${this
        .appSource}&bizdate=${this.formatedDate}`;
