<template>
  <div>
    <div class="searchTitle clearfix">
      ...
      <span class="urlTxt">URL:</span>
      <el-select class="s-select urlSelect" v-model="url" filterable placeholder="请选择">
        <el-option v-for="item in options" :key="item.rootUrl" :label="item.rootUrlName" :value="item.rootUrl">
        </el-option>
      </el-select>

      <el-date-picker v-model="dateTime" type="date" placeholder="选择日期" style="margin-left: 20px">
      </el-date-picker>
      <button type="button" class="btnQuery" @click="getFirstSiteAnalysis">确定</button>

    </div>

    <div class="ontimeChartContent" id="Accept_rateChart">
...
export default {
  ...
  methods: {
    ...
    async getQueryAppRootByOs() {
      try {
        let data = await queryAppRootByOs({
    ...
    async getFirstSiteAnalysis() {
      try {
        if (!this.checkParams()) return;
        let data = await firstSiteAnalysis({
          bizdate: this.formatedDate,
          rootUrl: this.url,
          appSource: this.appSource,
          os: this.headerTapOs,
          datecnt: this.timeRang,
          step: 1
        });
        // console.log(data);
        if (data.success) {
    ...
    async getNextSiteAnalysis(
      ...
      try {
        if (!this.checkParams()) return;
        let data = await nextSiteAnalysis({
          bizdate: this.formatedDate,
          appSource: this.appSource,
          os: this.headerTapOs,
